# CO analysis using stepwise regression since lasso regression based GAM did not perform well in comparison to linear model
...
# variable identification using stepwise regression

library(MASS)

model <- lm(CO~.,data=training_data_co) # creating a linear model to include all variables
model.aic <- stepAIC(model,direction='both') # doing both forward and backward stepwise regression
#model.bic <- stepAIC(model,direction='both',k=log(nrow(training_data_co)))


# creating formula for the model
all.names<- names(training_data_co)
# unwanted_variables<-c("CO","fuel_id","drive_id","voc_id","class_id",)
required_variables<-c("trip_count","total_average_speed"," max_speed", 
                        "driving_average_speed"," deceleration_events_per_mile","total_stops", 
                        "average_stop_duration","average_climbing_rate","max_descending_rate", 
                        "average_kinetic_power_density_demand", "maximum_potential_power_density_demand", 
                        "total_potential_power_density_demand ","average_potential_power_density_demand" 
                        ,"total_aerodynamic_power_density_demand","total_rolling_power_density_demand", 
                        "characteristic_acceleration","characteristic_deceleration", 
                        "average_kinetic_power_density_regen","total_potential_power_density_regen")
...
summary(gam.fit_co)
plot(gam.fit_co,se=TRUE,pages=10)
...
spline_variables<-c("maximum_potential_power_density_demand","total_average_speed","trip_count","max_speed","driving_average_speed","deceleration_events_per_mile","total_stops","average_stop_duration","total_potential_power_density_demand","average_potential_power_density_demand","total_rolling_power_density_demand","characteristic_acceleration","characteristic_deceleration","average_kinetic_power_density_regen","total_potential_power_density_regen")
